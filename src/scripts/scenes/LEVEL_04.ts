...
export default class LEVEL_04 extends Phaser.Scene {
...
/* 		let x_off_set = 100
		...
		for (let i = 0; i < tilesHigh; i++){
			for(let j = 0; j <tilesWide; j++){
				...
				if (tile != null){
					if (tile.index === 395){
						this.key.push(this.createSpineObject(IDLE_KEY, KEYS_KEY[3] ,j * this.tileSize - 80 , i * this.tileSize - 60 , 0.8, 0.8))
